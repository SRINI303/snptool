...
#				SNPSCAN
#                                                  
# Author     : SRINI,NISTUL,PRAKASH
# Tool       : snpscan v1.2
# Usage      : python3 snpscan.py example.com
...
def helper():
        ...
        print("------------")
        print("\t./snpscan.py example.com: Scans the domain example.com.")
        print("\t./snpscan.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./snpscan.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./snpscan.py --update   : Updates the scanner to the latest version.")
        print("\t./snpscan.py --help     : Displays this help context.")
...
def logo():
    ...
    logo_ascii = """
                                                                  ...
                                                                  """+bcolors.ENDC+"""(THE AUTOMATED VULNERABILITY ASSESSMENT TOOL)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"snpscan received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/snpscan_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/snpscan_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/snpscan_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/snpscan_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/snpscan_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o snpscan_temp_wapiti"],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("snpscan is updating....Please wait.\n")
    spinner.start()
    ...
    if rs_internet_availability == 0:
        ...
        spinner.stop()
        sys.exit(1)
    cmd = 'sha1sum snpscan.py | grep .... | cut -c 1-40'
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of snpscan." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"snpscan successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    os.system('rm /tmp/snpscan* > /dev/null 2>&1') # Clearing previous scan files
    os.system('clear')
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"snpscan was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by snpscan."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. snpscan will still perform the rest of the tests. Install these tools to fully utilize the functionality of snpscan."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/snpscan_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit snpscan.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open(vulreport, "a") as report:
            while(rs_vul < len(rs_vul_list)):
                ...
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/snpscan_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+vulreport+bcolors.ENDC+" is available under the same directory snpscan resides.")
    ...
    os.system('rm /tmp/snpscan_te* > /dev/null 2>&1') # Clearing previous scan files
